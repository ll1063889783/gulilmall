...
public class AlipayTemplate {
    ...
    // 支付宝会悄悄的给我们发送一个请求，告诉我们支付成功的信息(支付宝异步通知我们的系统订单成功了，需要内网穿透)
    private  String notify_url="http://fsdfxdfsdfdl.net/payed/notify";

    // 页面跳转同步通知页面路径 需http://格式的完整路径，不能加?id=123这类自定义参数，必须外网可以正常访问
    //同步通知，支付成功，一般跳转到成功页(跳转至会员服务的订单列表页面orderList页面)
